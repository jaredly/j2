...
export type Visitor<Ctx> = {
    ...
    IfConds?: (
        node: IfConds,
        ctx: Ctx,
    ) => null | false | IfConds | [IfConds | null, Ctx];
    IfCondsPost?: (node: IfConds, ctx: Ctx) => null | IfConds;
    IfCond?: (
        node: IfCond,
        ctx: Ctx,
    ...
    PArrayItems?: (
        node: PArrayItems,
        ctx: Ctx,
    ) => null | false | PArrayItems | [PArrayItems | null, Ctx];
    PArrayItemsPost?: (node: PArrayItems, ctx: Ctx) => null | PArrayItems;
    PArrayItem?: (
        node: PArrayItem,
        ctx: Ctx,
    ...
    TypeToplevel_Aliases?: (
        node: Aliases,
        ctx: Ctx,
    ) => null | false | TypeToplevel | [TypeToplevel | null, Ctx];
    TypeToplevelPost_Aliases?: (node: Aliases, ctx: Ctx) => null | TypeToplevel;
    ...
    AllTaggedTypes_IfYes?: (
        node: IfYes,
        ctx: Ctx,
    ) => null | false | AllTaggedTypes | [AllTaggedTypes | null, Ctx];
    AllTaggedTypesPost_IfYes?: (node: IfYes, ctx: Ctx) => null | AllTaggedTypes;
    AllTaggedTypes_IfConds?: (
        node: IfConds,
        ctx: Ctx,
    ) => null | false | AllTaggedTypes | [AllTaggedTypes | null, Ctx];
    AllTaggedTypesPost_IfConds?: (
        node: IfConds,
        ctx: Ctx,
    ) => null | AllTaggedTypes;
    ...
    AllTaggedTypes_PArray?: (
        node: PArray,
        ctx: Ctx,
    ) => null | false | AllTaggedTypes | [AllTaggedTypes | null, Ctx];
    AllTaggedTypesPost_PArray?: (
        node: PArray,
        ctx: Ctx,
    ) => null | AllTaggedTypes;
    AllTaggedTypes_PArrayItems?: (
        node: PArrayItems,
        ctx: Ctx,
    ) => null | false | AllTaggedTypes | [AllTaggedTypes | null, Ctx];
    AllTaggedTypesPost_PArrayItems?: (
        node: PArrayItems,
        ctx: Ctx,
    ) => null | AllTaggedTypes;
    AllTaggedTypes_PSpread?: (
        node: PSpread,
        ctx: Ctx,
    ) => null | false | AllTaggedTypes | [AllTaggedTypes | null, Ctx];
    AllTaggedTypesPost_PSpread?: (
        node: PSpread,
        ctx: Ctx,
    ) => null | AllTaggedTypes;
    ...
    AllTaggedTypes_PEnum?: (
        node: PEnum,
        ctx: Ctx,
    ) => null | false | AllTaggedTypes | [AllTaggedTypes | null, Ctx];
    AllTaggedTypesPost_PEnum?: (node: PEnum, ctx: Ctx) => null | AllTaggedTypes;
...
export const transformPEnum = <Ctx>(
    ...
    let updatedNode = node;
    {
        ...
        if (updatedNode$payload$current != null) {
            const updatedNode$payload$1$ = transformPTuple(
...
export const transformPArrayItems = <Ctx>(
    node: PArrayItems,
    visitor: Visitor<Ctx>,
    ctx: Ctx,
): PArrayItems => {
    if (!node) {
        throw new Error('No PArrayItems provided');
    }

    const transformed = visitor.PArrayItems
        ? visitor.PArrayItems(node, ctx)
        : null;
    if (transformed === false) {
        return node;
    }
    ...
    node = updatedNode;
    if (visitor.PArrayItemsPost) {
        const transformed = visitor.PArrayItemsPost(node, ctx);
        if (transformed != null) {
            node = transformed;
        }
    }
...
export const transformPArray = <Ctx>(
    ...
    let updatedNode = node;
    {
        ...
        const updatedNode$items$current = node.items;
        if (updatedNode$items$current != null) {
            const updatedNode$items$1$ = transformPArrayItems(
                updatedNode$items$current,
                visitor,
                ctx,
            );
...
export const transformIfConds = <Ctx>(
    node: IfConds,
    visitor: Visitor<Ctx>,
    ctx: Ctx,
): IfConds => {
    if (!node) {
        throw new Error('No IfConds provided');
    }

    const transformed = visitor.IfConds ? visitor.IfConds(node, ctx) : null;
    if (transformed === false) {
        return node;
    }
    if (transformed != null) {
    ...
    node = updatedNode;
    if (visitor.IfCondsPost) {
        const transformed = visitor.IfCondsPost(node, ctx);
        if (transformed != null) {
            node = transformed;
        }
    }
...
export const transformIfYes = <Ctx>(
    ...
    let updatedNode = node;
    {
        ...
        const updatedNode$conds = transformIfConds(node.conds, visitor, ctx);
        changed1 = changed1 || updatedNode$conds !== node.conds;
...
export const transformTypeToplevel = <Ctx>(
    ...
    switch (node.type) {
        case 'Aliases': {
            const transformed = visitor.TypeToplevel_Aliases
                ? visitor.TypeToplevel_Aliases(node, ctx)
                : null;
            if (transformed != null) {
                if (Array.isArray(transformed)) {
                    ctx = transformed[1];
    ...
    switch (updatedNode.type) {
        case 'Aliases': {
            const transformed = visitor.TypeToplevelPost_Aliases
                ? visitor.TypeToplevelPost_Aliases(updatedNode, ctx)
                : null;
            if (transformed != null) {
                updatedNode = transformed;
            }
...
export const transformAllTaggedTypes = <Ctx>(
    ...
    switch (node.type) {
        ...
        case 'IfYes': {
            const transformed = visitor.AllTaggedTypes_IfYes
                ? visitor.AllTaggedTypes_IfYes(node, ctx)
                : null;
            if (transformed != null) {
                if (Array.isArray(transformed)) {
                    ctx = transformed[1];
        ...
        case 'IfConds': {
            const transformed = visitor.AllTaggedTypes_IfConds
                ? visitor.AllTaggedTypes_IfConds(node, ctx)
                : null;
            if (transformed != null) {
                if (Array.isArray(transformed)) {
                    ctx = transformed[1];
        ...
        case 'PArray': {
            const transformed = visitor.AllTaggedTypes_PArray
                ? visitor.AllTaggedTypes_PArray(node, ctx)
                : null;
            if (transformed != null) {
                if (Array.isArray(transformed)) {
                    ctx = transformed[1];
        ...
        case 'PArrayItems': {
            const transformed = visitor.AllTaggedTypes_PArrayItems
                ? visitor.AllTaggedTypes_PArrayItems(node, ctx)
                : null;
            if (transformed != null) {
                if (Array.isArray(transformed)) {
                    ctx = transformed[1];
        ...
        case 'PSpread': {
            const transformed = visitor.AllTaggedTypes_PSpread
                ? visitor.AllTaggedTypes_PSpread(node, ctx)
                : null;
            if (transformed != null) {
                if (Array.isArray(transformed)) {
                    ctx = transformed[1];
        ...
        case 'PEnum': {
            const transformed = visitor.AllTaggedTypes_PEnum
                ? visitor.AllTaggedTypes_PEnum(node, ctx)
                : null;
            if (transformed != null) {
                if (Array.isArray(transformed)) {
                    ctx = transformed[1];
    ...
    switch (node.type) {
        ...
        case 'IfYes': {
            updatedNode = transformIfYes(node, visitor, ctx);
            changed0 = changed0 || updatedNode !== node;
            break;
        }

        case 'IfConds': {
            updatedNode = transformIfConds(node, visitor, ctx);
            changed0 = changed0 || updatedNode !== node;
            break;
        }
        ...
        case 'PArrayItems': {
            updatedNode = transformPArrayItems(node, visitor, ctx);
            changed0 = changed0 || updatedNode !== node;
            break;
        }
    ...
    switch (updatedNode.type) {
        ...
        case 'IfYes': {
            const transformed = visitor.AllTaggedTypesPost_IfYes
                ? visitor.AllTaggedTypesPost_IfYes(updatedNode, ctx)
                : null;
            if (transformed != null) {
                updatedNode = transformed;
            }
        ...
        case 'IfConds': {
            const transformed = visitor.AllTaggedTypesPost_IfConds
                ? visitor.AllTaggedTypesPost_IfConds(updatedNode, ctx)
                : null;
            if (transformed != null) {
                updatedNode = transformed;
            }
        ...
        case 'PArray': {
            const transformed = visitor.AllTaggedTypesPost_PArray
                ? visitor.AllTaggedTypesPost_PArray(updatedNode, ctx)
                : null;
            if (transformed != null) {
                updatedNode = transformed;
            }
        ...
        case 'PArrayItems': {
            const transformed = visitor.AllTaggedTypesPost_PArrayItems
                ? visitor.AllTaggedTypesPost_PArrayItems(updatedNode, ctx)
                : null;
            if (transformed != null) {
                updatedNode = transformed;
            }
        ...
        case 'PSpread': {
            const transformed = visitor.AllTaggedTypesPost_PSpread
                ? visitor.AllTaggedTypesPost_PSpread(updatedNode, ctx)
                : null;
            if (transformed != null) {
                updatedNode = transformed;
            }
        ...
        case 'PEnum': {
            const transformed = visitor.AllTaggedTypesPost_PEnum
                ? visitor.AllTaggedTypesPost_PEnum(updatedNode, ctx)
                : null;
            if (transformed != null) {
                updatedNode = transformed;
            }
